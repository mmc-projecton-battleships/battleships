...
void check_win();
void Init_map();
void eog();		//end of game

//variables
char timer[5];
char missiles;
short ships_count=0;
char ships[8][3];//2'd array of ships positions.
...
void insert_ships_to_map()
{
	for(int i=0;i<ships_count;i++)
		for(int j=0;j<3;j++)
		{
			map[ships[i][j]/16][ships[i][j]%16]='Y';//there is a ship
		}
}
void Init_Ships()
{
...
void screen_game()
{
	read_button = (GPIO->P[1].DIN & 0x00000200);
	if(read_button!=0)
		LCD_num++;
	if (LCD_num ==3)
		LCD_num=0;
	...
	{
	case 0:
		//#devnote:print time and missles left
		break;
	case 1:
		//#devnote:print upper half of map.
		break;
	case 2:
		//#devnote:print bottom half of map.
		break;
	}
	check_input();
	switch (input)
		...
		default:
			...
			for (int i=0;i<ships_count;i++)
			{
				...
				for(int j=0;j<3;j++)
				{
					...
					{
						hit='h';
						map[ships[i][j]/16][ships[i][j]%16]='X';
						ships[i][j]+=128;
						c_fallen++;
					}
				}
				if(c_fallen==3)//this ship is fully fallen
				{
					...
					for(int j=0;j<3;j++)
					{
						if (ships[i][j]>=128)
							...
							send_data(ships[i][j]);
						map[ships[i][j]/16][ships[i][j]%16]='S';
						ships[i][j]='x';//means ship was sent.
					}
					break;
				}
				if(hit=='h')
...
void check_win()
{
	for(int i=0;i<ships_count;i++)
		for(int j=0;j<3;j++)
		{
			if(ships[i][j]!='X'&&ships[i][j]!='e'&&ships[i][j]!='S')
				return;
		}
	win='w';
}
//make ships in size of 3 chars
void make_ships()
{
...
//check if the ship is valid. in not , makes it valid.
char valid_ship(char pos, int size, char dir)		//direction : 0=down, 1=up, 2=right, 3=left.
{
	//try all directions
	for (int i=0; i<4; i++)
	{
		if(check_vio_bounds(pos,size,dir))//ensure save bounds in this direction
			if(check_other_ships(pos,size,dir))
				return dir+1;
		dir++;
		if(dir==4)
			dir=0;
...
void Init()
{
	...
	GPIO->P[1].DOUT = 1<<9;							// define pull up
	read_button = (GPIO->P[1].DIN & 0x00000200);
	...
	TIMER0->TOP= 0xB71B;							//timer counts untill B71B
